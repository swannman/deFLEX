# flexdec — offline batch decoder & A/B research

The **offline** side of flexdec: the from-scratch FLEX decoder (`flexdec.py`,
at the repo root) run as a **batch research / A-B tool** over a frozen `.cfile`
...
The **live streaming receiver** (see `../online/`) closes most of this gap by
running continuously over an unbounded stream rather than a 120 s window.
...
this is what `compare_ab.py` does): raw distinct-message counts come out fd 96 /
mm 126, but the multimon number is **inflated by its lack of FEC** — e.g. on
929.9375 its "18 distinct" is really ~7 real messages + ~9 garbled copies of a
single trauma alert (so corrupted they don't even cluster with each other) + 2
noise lines. flexdec's are clean and deduplicated.
...
# flexdec.py lives at the repo root:
...
## A/B comparison harness (`compare_ab.py`)

Per-carrier unique-readable-alpha A/B of flexdec vs multimon-ng. Reads
`fd_<off>.txt` (flexdec `READABLE<TAB><repr>` dumps) and `mm_<off>.txt`
(multimon `FLEX_NEXT|...|ALN|...` lines), normalizes both (collapse whitespace,
...
- **flexdec.py freeze was lifted 2026-05-29** to add numba acceleration
  (`flexdec_numba.py`); the decode is bit-exact vs the pure-Python baseline. See
  the root README and `../online/` for details.
